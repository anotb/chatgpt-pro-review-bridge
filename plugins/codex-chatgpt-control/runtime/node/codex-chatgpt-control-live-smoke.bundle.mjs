...
import { mkdtemp as mkdtemp2, readFile as readFile7, stat as stat9, writeFile as writeFile6 } from "node:fs/promises";
...
async function readPageSnapshot(page) {
  if (typeof page.evaluate === "function") {
    try {
      const value = await withTimeout2(page.evaluate((loginLabels) => {
...
async function readHtmlPageSnapshot(page) {
  ...
  try {
    const html = await withTimeout2(page.content(), 1e3, "Timed out while reading page content.");
...
async function readLegacyBlockerSurface(page) {
  ...
  try {
    const value = await withTimeout2(page.evaluate(() => {
      const messageSelector = "[data-message-author-role], [data-testid^='conversation-turn']";
      const systemSelector = [
        "[role='alert']",
        "[role='status']",
...
async function captureArtifactDelta(env, args) {
  ...
  if (!current.ok || current.data === void 0) {
    return forwardFailure2(current);
  }
  return resultOk(
...
async function inspectConfiguration(env, args = {}) {
  ...
  try {
    ...
    if (!detected.ok || detected.data === void 0) {
      return forwardFailure3(detected);
    }
...
async function applyConfiguration(env, args) {
  ...
  try {
    ...
    if (!beforeResult.ok || beforeResult.data === void 0) {
      return forwardFailure3(beforeResult);
    }
    ...
    if (!afterResult.ok || afterResult.data === void 0) {
      return forwardFailure3(afterResult);
    }
...
async function snapshotConfiguration(env, args = {}) {
  ...
  if (!inspected.ok || inspected.data === void 0) {
    return forwardFailure3(inspected);
  }
...
import { mkdtemp, readFile as readFile6, rename, rm as rm2, stat as stat8, writeFile as writeFile5 } from "node:fs/promises";
...
import { chmod, copyFile as copyFile3, link as link2, mkdir as mkdir5, readFile as readFile3, rm, stat as stat6, writeFile as writeFile4 } from "node:fs/promises";
...
async function createReviewArchive(repositoryRoot, archiveRoot, headSha, now = /* @__PURE__ */ new Date()) {
  ...
  for (let attempt = 0; attempt < 16; attempt += 1) {
    ...
    try {
      await mkdir5(path3, { recursive: false, mode: 448 });
    } catch (error) {
      if (error.code === "EEXIST") continue;
      throw error;
    }
    await mkdir5(join5(path3, "context"), { recursive: false, mode: 448 });
    await mkdir5(join5(path3, "artifacts"), { recursive: false, mode: 448 });
...
async function writeImmutableFile(path3, data) {
  await mkdir5(dirname2(path3), { recursive: true, mode: 448 });
  ...
  await writeFile4(temporary, expected, { flag: "wx", mode: 384 });
  try {
...
async function preserveDownloadedArtifact(downloadedPath, archiveDirectory, desiredName, used, metadata = {}) {
  ...
  const artifactsRoot = resolve3(archiveDirectory, "artifacts");
...
import { basename as basename4, dirname as dirname3, isAbsolute as isAbsolute2, join as join6, relative as relative2, resolve as resolve4, sep as sep2 } from "node:path";
...
function governingInstructions(tracked, changed) {
  ...
  return instructions.filter((instruction) => changed.some((path3) => {
    const scope = dirname3(instruction);
...
import { lstat as lstat2, mkdir as mkdir6, open, readFile as readFile5, readdir, rmdir, unlink as unlink2 } from "node:fs/promises";
...
`;
  ...
  for (let attempt = 0; attempt < 2; attempt += 1) {
    try {
      await mkdir6(leasePath, { mode: 448 });
