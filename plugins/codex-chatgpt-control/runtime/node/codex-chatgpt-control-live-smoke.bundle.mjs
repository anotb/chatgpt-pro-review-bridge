...
async function readBlockerSurface(page) {
  ...
  if (typeof page.content === "function") {
    try {
      const html = await withTimeout2(page.content(), 1e3, "Timed out while reading blocker surfaces.");
...
import { mkdtemp, open, readFile as readFile5, rename, rm as rm2, stat as stat8, writeFile as writeFile5 } from "node:fs/promises";
...
async function createReviewArchive(repositoryRoot, archiveRoot, headSha, now = /* @__PURE__ */ new Date()) {
  ...
  await mkdir5(join5(path3, "context"), { recursive: true, mode: 448 });
  await mkdir5(join5(path3, "artifacts"), { recursive: true, mode: 448 });
...
async function validationOutput(args, root) {
  ...
  const absolute = isAbsolute2(path3) ? resolve4(path3) : resolve4(root, path3);
  assertInside(root, absolute);
