    ...
    const fromDom = await withTimeout(
      page.evaluate(async (selector) => {
        ...
        if (/^(blob:|https?:)/i.test(src)) {
          ...
          const dataUrl = await new Promise((resolve7, reject) => {
...
async function sleep(page, ms2) {
  ...
  await new Promise((resolve7) => setTimeout(resolve7, ms2));
}
...
async function raceFileChooserOpen(chooserPromise, page, waitMs) {
  return Promise.race([
    ...
    (page.waitForTimeout?.(waitMs) ?? new Promise((resolve7) => setTimeout(resolve7, waitMs))).then(() => false)
  ]);
}
...
import { mkdtemp, readFile as readFile7, rename, rm as rm2, stat as stat8, writeFile as writeFile4 } from "node:fs/promises";
...
import { chmod, copyFile as copyFile3, link as link2, mkdir as mkdir4, readFile as readFile4, rm, stat as stat6, writeFile as writeFile3 } from "node:fs/promises";
import { basename as basename3, dirname as dirname2, isAbsolute, join as join4, relative, resolve as resolve4, sep } from "node:path";
async function sha256File2(path3) {
  return createHash5("sha256").update(await readFile4(path3)).digest("hex");
}
...
async function writeImmutableFile(path3, data) {
  ...
  await writeFile3(temporary, expected, { flag: "wx", mode: 384 });
  try {
    ...
    const existing = await readFile4(path3);
...
async function preserveDownloadedArtifact(downloadedPath, archiveDirectory, desiredName, used, metadata = {}) {
  ...
  await mkdir4(artifactsRoot, { recursive: true });
...
import { lstat, readFile as readFile5, realpath, stat as stat7 } from "node:fs/promises";
import { basename as basename4, dirname as dirname3, isAbsolute as isAbsolute2, join as join5, relative as relative2, resolve as resolve5, sep as sep2 } from "node:path";
...
async function readWorkingTreeCandidate(root, path3) {
  ...
  return await readFile5(absolute);
}
...
async function validationOutput(args, root, lexicalRoot) {
  ...
  const absolute = join5(await realpath(dirname3(supplied)), basename4(supplied));
  assertInside(root, absolute);
  ...
  return await readFile5(resolved, "utf8");
}
...
import { lstat as lstat2, mkdir as mkdir5, open, readFile as readFile6, readdir, rmdir, unlink as unlink2 } from "node:fs/promises";
...
async function removeLegacyLeaseTextIfUnchanged(leasePath, expected, expectedMtimeMs) {
  try {
    if (await readFile6(leasePath, "utf8") !== expected) return false;
    if (expectedMtimeMs !== void 0) {
...
async function leaseGenerationHeartbeatIsStale(leasePath, snapshot) {
  try {
    ...
    if (await readFile6(heartbeatPath, "utf8") !== snapshot.leaseText) return void 0;
...
async function archiveProvesSubmissionAlreadyOccurred(leasePath) {
  ...
  for (const filename of ["submission-confirmation.json", "submission.json"]) {
    let value;
    try {
      value = JSON.parse(await readFile6(join6(archiveDirectory, filename), "utf8"));
    } catch (error) {
...
async function readReviewLeaseSnapshot(leasePath) {
  ...
  if (!leaseStat.isDirectory()) {
    ...
    const leaseText2 = await readFile6(leasePath, "utf8");
    let value2;
    try {
  ...
  const leaseText = await readFile6(ownerPath, "utf8");
  let value;
  try {
...
async function removeLeaseIfOwnerExitedOrExpired(leasePath, probeLiveness = probeProcessLiveness) {
  ...
  if (snapshot === void 0) {
    try {
      ...
      const leaseText = await readFile6(leasePath, "utf8");
  ...
  if (snapshot.format === "directory") {
    if (await readFile6(snapshot.ownerPath, "utf8").catch(() => void 0) !== snapshot.leaseText) return false;
  ...
  try {
    ...
    if (isPreSubmitAttempt) {
      ...
      if (archiveDirectory !== void 0) {
        ...
        await writeImmutableJson(join7(archiveDirectory, "submission.json"), archivedSubmissionRecord);
    ...
    const archivedResponse = archiveDirectory === void 0 ? void 0 : await readFile7(join7(archiveDirectory, "response.md"), "utf8").catch(() => void 0);
  ...
  try {
    if (archiveDirectory !== void 0 && !terminalOutcomeAlreadyFinal && (releaseLease !== void 0 || result.blocker?.code !== "review_archive_locked")) {
      ...
      try {
        ...
        try {
          await writeImmutableJson(join7(archiveDirectory, "archive-commit-failure.json"), archiveCommitFailure);
        } catch (markerError) {
...
async function readArchivedConfigurationSnapshot(archiveDirectory) {
  const value = JSON.parse(await readFile7(join7(archiveDirectory, "configuration.before.json"), "utf8"));
...
async function readArchivedPreparedContext(archiveDirectory) {
  ...
  let prompt;
  try {
    prompt = await readFile7(promptPath, "utf8");
  } catch {
...
async function readArchivedArtifactBaseline(archiveDirectory) {
  const submission = JSON.parse(await readFile7(join7(archiveDirectory, "submission.json"), "utf8"));
...
async function archivedSubmissionRecordExists(archiveDirectory) {
  for (const name of ["submission-confirmation.json", "submission.json", "submission-intent.json"]) {
    if (await stat8(join7(archiveDirectory, name)).then((entry) => entry.isFile()).catch(() => false)) return true;
  }
  return false;
}
...
async function readArchivedTerminalOutcome(archiveDirectory) {
  const value = await readArchivedTerminalMarker(
    join7(archiveDirectory, "terminal-outcome.json"),
...
async function readArchivedCommitFailure(archiveDirectory) {
  const value = await readArchivedTerminalMarker(
    join7(archiveDirectory, "archive-commit-failure.json"),
...
async function validateArchivedTerminalBinding(archiveDirectory, outcome, submission) {
  ...
  if (outcome.response !== void 0) {
    const response = await readFile7(join7(archiveDirectory, "response.md"), "utf8").catch(() => void 0);
...
async function archivedFinalMarkerExists(archiveDirectory) {
  ...
  for (const marker of markers) {
    if (await stat8(join7(archiveDirectory, marker)).then(() => true).catch(() => false)) return true;
  }
  return false;
}
...
async function readArchivedThreadCheckpoint(archiveDirectory) {
  const value = JSON.parse(await readFile7(join7(archiveDirectory, "thread-checkpoint.json"), "utf8"));
...
async function readArtifactDownloadCheckpoint(archiveDirectory) {
  ...
  let value;
  try {
    value = JSON.parse(await readFile7(checkpointPath, "utf8"));
  } catch (error) {
    if (error.code !== "ENOENT") throw error;
    try {
      value = JSON.parse(await readFile7(manifestPath, "utf8"));
    } catch (manifestError) {
...
var StdioBackendTransport = class {
  ...
  async request(request) {
    ...
    return new Promise((resolve7, reject) => {
      ...
      this.pendingResponses.set(requestId, { resolve: resolve7, reject, timeout });
...
function streamFromBackendEvents(events) {
  ...
  let rejectCompleted;
  const completed = new Promise((resolve7, reject) => {
...
function runPlanPayload(payload) {
  if (isRecord11(payload.plan)) return payload.plan;
  return payload;
}
function commandFilter(payload) {
  if (isRecord11(payload.filter)) return payload.filter;
...
export {
  ...
  configurationSelectionFromInspection,
  contextNow,
  copyResponse,
  copyResponseButtons,
  countMessages,
  ...
  inspectGeneratedFileAffordances,
