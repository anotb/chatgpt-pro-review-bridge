...
async function createReviewArchive(repositoryRoot, archiveRoot, headSha, now = /* @__PURE__ */ new Date()) {
  ...
  const path3 = join5(root, `${timestamp}-${suffix}`);
  await mkdir6(join5(path3, "context"), { recursive: true, mode: 448 });
  await mkdir6(join5(path3, "artifacts"), { recursive: true, mode: 448 });
...
\`\`\``,
    ...
    if (secretFindings.some((item) => item.action === "blocked")) {
      ...
      const manifestPath2 = join6(archiveDirectory, "context", "manifest.json");
    ...
    if (args.resume === void 0) {
      ...
      if (submissionState !== "confirmed") {
        throw workflowBlocker(
          ...
          submissionState === "ambiguous" ? "ChatGPT showed possible submission progress, but the exact visible user turn could not be proven. The prompt will not be resent." : "The single allowed submit attempt did not produce a matching visible user turn. The prompt will not be resent automatically.",
          "SUBMIT_ONCE"
        );
      }
...
async function readArchivedPreparedContext(archiveDirectory) {
  ...
  return {
    ...
    promptPath,
    packetPaths: manifest.packets.map((packet) => join7(archiveDirectory, "context", packet.path)),
    manifestPath,
    manifest,
