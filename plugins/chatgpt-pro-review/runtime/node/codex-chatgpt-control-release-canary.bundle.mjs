...
import { mkdtemp as mkdtemp2, readFile as readFile6, stat as stat8, writeFile as writeFile7 } from "node:fs/promises";
...
import { mkdtemp, readFile as readFile5, rename as rename2, rm, writeFile as writeFile6 } from "node:fs/promises";
import { tmpdir } from "node:os";
import { join as join7 } from "node:path";

// src/reviews/archive.ts
import { createHash as createHash5, randomBytes } from "node:crypto";
import { copyFile as copyFile3, mkdir as mkdir6, readFile as readFile3, rename, stat as stat6, writeFile as writeFile5 } from "node:fs/promises";
...
async function writeImmutableFile(path3, data) {
  ...
  await writeFile5(temporary, data, { flag: "wx" });
...
async function preserveDownloadedArtifact(downloadedPath, archiveDirectory, desiredName, used, metadata = {}) {
  ...
  if (resolve4(downloadedPath) !== target) await copyFile3(downloadedPath, target);
...
async function runReleaseCanary(runtime, options) {
  ...
  const reportDir = resolve6(options.reportDir ?? join9(process.cwd(), "reports", "release-canary"));
