...
import { mkdtemp, readFile as readFile5, rename, rm as rm2, stat as stat8, writeFile as writeFile6 } from "node:fs/promises";
...
import { copyFile as copyFile3, link as link2, mkdir as mkdir6, readFile as readFile3, rm, stat as stat6, writeFile as writeFile5 } from "node:fs/promises";
...
async function createReviewArchive(repositoryRoot, archiveRoot, headSha, now = /* @__PURE__ */ new Date()) {
  ...
  await mkdir6(join5(path3, "context"), { recursive: true });
  await mkdir6(join5(path3, "artifacts"), { recursive: true });
  return path3;
}
async function writeImmutableFile(path3, data) {
  ...
  await writeFile5(temporary, expected, { flag: "wx" });
  try {
...
async function runCodeReviewWithPort(args, port) {
  ...
  if (archiveDirectory !== void 0) {
    ...
    await writeJsonReplacing(join7(archiveDirectory, "configuration.json"), configurationRecord).catch((error) => result.warnings.push(`Could not archive configuration evidence: ${String(error)}`));
    const receipt = {
      ...result,
    ...
    await writeJsonReplacing(join7(archiveDirectory, "receipt.json"), receipt).catch((error) => result.warnings.push(`Could not archive receipt: ${String(error)}`));
    await writeJsonReplacing(join7(archiveDirectory, "run-report.redacted.json"), redactReportValue(receipt)).catch((error) => result.warnings.push(`Could not archive redacted run report: ${String(error)}`));
  }
