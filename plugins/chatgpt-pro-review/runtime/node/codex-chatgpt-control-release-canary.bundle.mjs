...
async function selectExistingUserTab(browser, policy, collectDiagnostics, exactTimeoutMs, strictOpenTabsErrors = false) {
  ...
  const target = policy.target;
  const claimCandidates = target?.type === "conversationId" && policy.ifMissing === "block" && policy.ifMultiple === "first" && policy.requireChatGPT === true ? matches.slice(0, 2) : [matches[0]];
...
async function snapshotConfiguration(env, args = {}) {
  const inspected = await inspectConfiguration(env, {
    ...args,
    includeOptions: false
  });
  if (!inspected.ok || inspected.data === void 0) {
...
async function restoreConfiguration(env, args) {
  const boot2 = await ensurePage(env);
  if (!boot2.ok) {
    return boot2;
  }
  ...
  try {
    ...
    const data = {
      ...
      restored: applied.data.verified && configurationMatchesSelection(applied.data.after, args.snapshot.selection)
    };
    if (!data.restored) {
      return configurationRestoreFailure(
        page,
...
function configurationSelectionFromInspection(inspection) {
  const selection = {};
...
async function configurationRestoreFailure(page, data, source, code, message) {
  const after = data.after ?? data.applied?.after;
  const candidates = after === void 0 ? source?.blocker?.candidates : Object.entries(after.active).map(([axis, label]) => ({ label: `${axis}: ${label}` }));
  return {
    ok: false,
    status: "blocked",
    data,
    ...
    context: await contextFromPage(page, after === void 0 ? {} : {
      experience: after.experience,
      selectorProfile: after.selectorProfile
    })
  };
...
import { mkdtemp as mkdtemp2, readFile as readFile7, stat as stat9, writeFile as writeFile7 } from "node:fs/promises";
...
async function readImageDataUrl(page, timeoutMs, which = "latest") {
  ...
  if (typeof page.evaluate === "function") {
    const fromDom = await withTimeout(
      page.evaluate(async (selector) => {
        ...
        const image = selector === "latest" ? candidates.at(-1) : candidates[selector.index];
        if (image !== void 0 && selector !== "latest" && selector.turnId !== void 0 && image.closest("[data-testid^='conversation-turn']")?.getAttribute("data-testid") !== selector.turnId) {
          return void 0;
        }
  ...
  if (which !== "latest" && which.turnId !== void 0) return void 0;
...
async function readWaitDomSnapshot(page) {
  ...
  return page.evaluate((args) => {
    ...
    for (let index = 0; index < normalizedText.length; index += 1) {
      hash2 ^= normalizedText.charCodeAt(index);
      hash2 = Math.imul(hash2, 16777619);
    }
    const textHash = (hash2 >>> 0).toString(16).padStart(8, "0");
...
async function tryDownloadControl(page, args, timeoutMs) {
  try {
    ...
    const target = args.which !== void 0 && args.which !== "latest" && controls.nth !== void 0 ? controls.nth(args.which.index) : controls.last?.() ?? controls;
...
async function saveVisibleImageSource(page, destDir, timeoutMs, which) {
...
async function savePageAssetImage(env, page, destDir, timeoutMs, which) {
  return await savePageAssetImageFromPage(page, destDir, timeoutMs, which).catch(() => void 0) ?? await withTemporaryBridgeOwnedPage(env, page, timeoutMs, async (freshPage) => {
    return await savePageAssetImageFromPage(freshPage, destDir, timeoutMs, which).catch(() => void 0);
  });
}
async function savePageAssetImageFromPage(page, destDir, timeoutMs, which) {
  ...
  try {
    ...
    if (!readiness.ready) {
      ...
      const blocker = {
        ...
        message: verificationFailed ? "ChatGPT did not expose composer-scoped evidence for every requested attachment, so the prompt was not submitted." : "ChatGPT still appears to be processing the attached file, so the prompt was not submitted.",
...
async function readAttachmentReadiness(page, files) {
  ...
  return page.evaluate((expectedFiles) => {
    ...
    const composer = input?.closest("form, [data-testid*='composer' i], [class*='composer' i]") ?? document.querySelector("form[data-type='unified-composer'], form:has(textarea), form:has([contenteditable='true'])");
    ...
    const visibleFiles = expectedFiles.map((expected) => {
      ...
      const inputIndex = inputFiles.findIndex(
        (candidate, index) => !usedInputs.has(index) && candidate.name === expectedName && candidate.size === expected.bytes
      );
      if (inputIndex >= 0) {
      ...
      const labelIndex = attachmentLabels.findIndex(
        (label, index) => !usedLabels.has(index) && label.includes(expectedName)
      );
...
// src/commands/artifact-inventory.ts
async function captureArtifactBaseline(env, args = {}) {
  const boot2 = await ensurePage(env);
  if (!boot2.ok) return boot2;
  const page = env.page;
  const images = await listLatestArtifacts(env, {
    kind: "image",
    max: args.maxImages ?? 1e3,
    ...args.timeoutMs === void 0 ? {} : { timeoutMs: args.timeoutMs }
  });
  if (!images.ok || images.data === void 0) {
    return forwardFailure3(images);
  }
  try {
    const files = await inspectGeneratedFileAffordances(
      page,
      localGuardTimeout(args.timeoutMs, 5e3)
    );
...
function diffArtifactInventories(baseline, current) {
...
function artifactInventoryItems(images, files) {
  return [
    ...images.map((artifact) => ({
      key: stableKey(artifact.turnId === void 0 ? {
        kind: "image",
...
function forwardFailure3(result) {
  const forwarded = {
    ok: false,
    status: result.status,
    warnings: result.warnings,
...
async function raceFileChooserOpen(chooserPromise, page, waitMs) {
  return Promise.race([
    ...
    (page.waitForTimeout?.(waitMs) ?? new Promise((resolve8) => setTimeout(resolve8, waitMs))).then(() => false)
  ]);
}
...
import { mkdtemp, readFile as readFile6, rename, rm as rm2, stat as stat8, writeFile as writeFile6 } from "node:fs/promises";
...
import { chmod, copyFile as copyFile3, link as link2, mkdir as mkdir6, readFile as readFile3, rm, stat as stat6, writeFile as writeFile5 } from "node:fs/promises";
import { basename as basename3, dirname as dirname3, isAbsolute, join as join5, relative, resolve as resolve4, sep } from "node:path";
...
async function createReviewArchive(repositoryRoot, archiveRoot, headSha, now = /* @__PURE__ */ new Date()) {
  const root = isAbsolute(archiveRoot) ? resolve4(archiveRoot) : resolve4(repositoryRoot, archiveRoot);
  const timestamp = now.toISOString().replace(/[:.]/g, "-");
  const revision = headSha?.slice(0, 12) ?? "unborn";
  await mkdir6(root, { recursive: true, mode: 448 });
  for (let attempt = 0; attempt < 16; attempt += 1) {
    const path3 = join5(root, `${timestamp}-${revision}-${randomBytes(6).toString("hex")}`);
    try {
      await mkdir6(path3, { recursive: false, mode: 448 });
    } catch (error) {
      if (error.code === "EEXIST") continue;
      throw error;
    }
    await mkdir6(join5(path3, "context"), { recursive: false, mode: 448 });
    await mkdir6(join5(path3, "artifacts"), { recursive: false, mode: 448 });
    await Promise.all([path3, join5(path3, "context"), join5(path3, "artifacts")].map((directory) => chmod(directory, 448)));
    return path3;
  }
  throw new Error("Unable to allocate an exclusive review archive directory.");
}
async function writeImmutableFile(path3, data) {
  await mkdir6(dirname3(path3), { recursive: true, mode: 448 });
  ...
  await writeFile5(temporary, expected, { flag: "wx", mode: 384 });
  try {
    ...
    if (["EPERM", "ENOSYS", "EXDEV", "EOPNOTSUPP"].includes(code ?? "")) {
      try {
        await copyFile3(temporary, path3, constants3.COPYFILE_EXCL);
      } catch (copyError) {
...
function sanitizeArtifactFilename(name, fallback = "artifact") {
  const leaf = basename3(name.replace(/\\/g, "/"));
  const cleaned = leaf.normalize("NFKC").replace(/[<>:"/\\|?*\u0000-\u001f]/g, "_").replace(/[. ]+$/g, "").trim();
  const usable = cleaned.length > 0 && cleaned !== "." && cleaned !== ".." ? cleaned : fallback;
  return usable.slice(0, 180);
}
function collisionSafeFilename(name, used) {
...
async function preserveDownloadedArtifact(downloadedPath, archiveDirectory, desiredName, used, metadata = {}) {
  const name = collisionSafeFilename(desiredName, used);
  const artifactsRoot = resolve4(archiveDirectory, "artifacts");
  const target = resolve4(artifactsRoot, name);
  assertPathInside(artifactsRoot, target);
  await mkdir6(artifactsRoot, { recursive: true });
  if (resolve4(downloadedPath) !== target) {
    try {
  ...
  const artifact = {
    ...
    sha256: await sha256File2(target)
  };
...
function markdownSectionIndex(markdown) {
  const entries = [];
...
import { lstat, readFile as readFile4, realpath, stat as stat7 } from "node:fs/promises";
import { basename as basename4, dirname as dirname4, isAbsolute as isAbsolute2, join as join6, relative as relative2, resolve as resolve5, sep as sep2 } from "node:path";
...
${fencedBlock("text", numbered)}`,
    ...
    for (const [index, packet] of serializedPackets.entries()) {
      ...
      const path3 = join6(archiveDirectory, "context", name);
...
`);
    ...
    const uploadManifestPath = join6(archiveDirectory, "context", "manifest.upload.json");
    ...
    const requestPath = join6(archiveDirectory, "request.md");
...
async function resolveRepositoryRoots(input) {
  const candidate = resolve5(input);
  ...
  const lexical = prefix.length === 0 ? candidate : resolve5(candidate, ...prefix.split("/").filter(Boolean).map(() => ".."));
...
async function workingTreePaths(root, archivePathPrefix) {
  ...
  const present = await Promise.all(candidates.map(async (path3) => {
    const absolute = resolve5(root, path3);
...
function packetPathspec(excludedPaths, archivePathPrefix) {
...
function governingInstructions(tracked, changed) {
  ...
  return instructions.filter((instruction) => changed.some((path3) => {
    const scope = dirname4(instruction);
    return scope === "." || path3 === scope || path3.startsWith(`${scope}/`);
  }));
}
function affectsChangedPath(path3, changed) {
  const dir = dirname4(path3);
...
async function gitChecked(root, args, code, allowedNonzero = []) {
...
function repositoryRelativeArchivePrefix(root, archiveRoot) {
  ...
  const rel = relative2(resolve5(root), absolute);
...
function excludedPathReason(path3, archivePathPrefix) {
  const normalized = normalizeRepoPath(path3);
  if (archivePathPrefix !== void 0 && (normalized === archivePathPrefix || normalized.startsWith(`${archivePathPrefix}/`))) {
...
import { lstat as lstat2, mkdir as mkdir7, open, readFile as readFile5, readdir, rmdir, unlink as unlink2 } from "node:fs/promises";
...
`;
  ...
  for (let attempt = 0; attempt < 2; attempt += 1) {
    try {
      await mkdir7(leasePath, { mode: 448 });
      ownerPath = join7(leasePath, `${leaseId}.json`);
      break;
    } catch (error) {
      const code = error.code;
...
async function archiveProvesSubmissionAlreadyOccurred(leasePath) {
  const archiveDirectory = dirname5(leasePath);
  for (const filename of ["submission-confirmation.json", "submission.json"]) {
    let value;
    try {
      value = JSON.parse(await readFile5(join7(archiveDirectory, filename), "utf8"));
    } catch (error) {
...
async function readReviewLeaseSnapshot(leasePath) {
  ...
  const ownerPath = join7(leasePath, entries[0]);
  if (dirname5(ownerPath) !== leasePath || !(await lstat2(ownerPath)).isFile()) return void 0;
...
async function removeLeaseIfOwnerExitedOrExpired(leasePath, probeLiveness = probeProcessLiveness) {
  ...
  if (snapshot.format === "directory") {
    if (typeof value.leaseId !== "string" || snapshot.ownerPath !== join7(leasePath, `${value.leaseId}.json`)) return false;
  }
...
var ReviewInProgress = class extends Error {
  constructor() {
...
async function runCodeReviewWithPort(args, port) {
  ...
  const runStep = async (state, operation) => {
    ...
    try {
      ...
      if (archiveDirectory !== void 0) {
        await writeJsonReplacing(join8(archiveDirectory, "workflow-progress.json"), {
  ...
  try {
    ...
    const boot2 = requireOk(await runStep("PREFLIGHT_BROWSER", async () => {
      if (isPreSubmitAttempt && requestedThreadTarget !== void 0 && archiveDirectory !== void 0 && !preSubmitCheckpointActive) {
        await writeImmutableJson(
          join8(archiveDirectory, "pre-submit-checkpoint.json"),
    ...
    if (archiveDirectory !== void 0 && (args.resume === void 0 || preSubmitResume)) {
      await rm2(join8(archiveDirectory, "pre-submit-checkpoint.json"), { force: true });
    ...
    if (archiveDirectory !== void 0 && isPreSubmitAttempt) {
      await writeImmutableJson(join8(archiveDirectory, "configuration.before.json"), configurationBefore);
    }
    let appliedData;
    ...
    if (isPreSubmitAttempt) {
      ...
      if (submitResult?.ok !== true && submitError !== void 0) {
      ...
      await runStep("DOWNLOAD_AND_HASH_ARTIFACTS", async () => {
        ...
        try {
          for (const item of delta.added) {
            ...
            await writeJsonReplacing(join8(artifactArchiveDirectory, "artifacts", "download-checkpoint.json"), {
              schemaVersion: 1,
  ...
  if (preSubmitCheckpointActive && archiveDirectory !== void 0 && blocker?.code !== "existing_tab_handoff_completed") {
    await rm2(join8(archiveDirectory, "pre-submit-checkpoint.json"), { force: true }).catch(() => void 0);
    preSubmitCheckpointActive = false;
  }
  ...
  if (responseSha256 !== void 0) provenance.responseSha256 = responseSha256;
  const result = {
    ok: (terminalStatus === "completed" || terminalStatus === "completed_with_warnings") && blocker?.kind !== "configuration_restore_failed",
  ...
  if (configurationBefore !== void 0) result.configuration.before = configurationBefore;
  if (applied !== void 0) result.configuration.applied = applied;
  if (archiveDirectory !== void 0) result.archiveDirectory = archiveDirectory;
  if (threadUrl !== void 0 || threadId !== void 0) result.thread = { ...threadUrl === void 0 ? {} : { url: threadUrl }, ...threadId === void 0 ? {} : { id: threadId } };
  if (blocker !== void 0) result.blocker = blocker;
  if (terminalStatus === "in_progress") result.nextAction = "poll_same_thread";
  if (responseMarkdown !== void 0 && (terminalStatus === "completed" || terminalStatus === "completed_with_warnings")) {
    const mode = args.output?.mode ?? "full";
    const hardLimit = args.output?.hardTransportLimitBytes;
    const overHardLimit = hardLimit !== void 0 && Buffer.byteLength(responseMarkdown) > hardLimit;
    if (mode === "full" && !overHardLimit && args.output?.returnFullMarkdown !== false) {
      ...
      if (overHardLimit) {
        result.warnings.push(`The complete response is archived at ${join8(archiveDirectory ?? "", "response.md")}, but its ${Buffer.byteLength(responseMarkdown)} bytes exceed the explicitly configured hard transport limit of ${hardLimit}. No content was summarized or truncated.`);
  ...
  try {
    if (archiveDirectory !== void 0 && !terminalOutcomeAlreadyFinal && (releaseLease !== void 0 || result.blocker?.code !== "review_archive_locked")) {
      ...
      const receipt = {
        ...result,
        responseMarkdown: responseMarkdown === void 0 ? void 0 : { bytes: Buffer.byteLength(responseMarkdown), sha256: responseSha256 },
...
function requireData(result, state) {
  if (!result.ok || result.data === void 0) throw new ReviewWorkflowError(result, state);
  return result;
}
function workflowBlocker(kind, code, message, state) {
  return new ReviewWorkflowError({
    ok: false,
    status: "blocked",
    warnings: [],
...
function isCommandResult2(value) {
  return typeof value === "object" && value !== null && typeof value.ok === "boolean" && typeof value.status === "string";
}
...
async function readArchivedConfigurationSnapshot(archiveDirectory) {
  const value = JSON.parse(await readFile6(join8(archiveDirectory, "configuration.before.json"), "utf8"));
...
async function readArchivedPreparedContext(archiveDirectory) {
  const manifestPath = join8(archiveDirectory, "context", "manifest.json");
  let manifest;
  try {
  ...
  const promptPath = join8(archiveDirectory, "prompt.md");
  const contextDirectory = resolve6(archiveDirectory, "context");
  const packetPaths = [];
  for (const packet of manifest.packets) {
    ...
    const packetPath = resolve6(contextDirectory, packet.path);
    try {
  ...
  const candidateUploadManifestPath = join8(contextDirectory, "manifest.upload.json");
  ...
  return {
    ...
    archiveDirectory,
    requestPath: join8(archiveDirectory, "request.md"),
    promptPath,
    packetPaths,
    manifestPath,
    uploadManifestPath,
...
async function readArchivedArtifactBaseline(archiveDirectory) {
  const submission = JSON.parse(await readFile6(join8(archiveDirectory, "submission.json"), "utf8"));
...
async function readArchivedPreSubmitCheckpoint(archiveDirectory, prepared) {
  let value;
  try {
    value = await readOptionalJson(join8(archiveDirectory, "pre-submit-checkpoint.json"));
  } catch (error) {
    throw new ReviewPreparationError(
...
async function readArchivedSubmission(archiveDirectory, prepared) {
  const confirmation = await readOptionalJson(join8(archiveDirectory, "submission-confirmation.json"));
  const submittedRecord = confirmation ?? await readOptionalJson(join8(archiveDirectory, "submission.json"));
  const intentOnly = submittedRecord === void 0;
  const value = submittedRecord ?? await readOptionalJson(join8(archiveDirectory, "submission-intent.json"));
  if (value === void 0) {
...
async function persistThreadCheckpoint(archiveDirectory, prepared, url, id2, tabId, now) {
  ...
  await writeJsonReplacing(join8(archiveDirectory, "thread-checkpoint.json"), checkpoint);
}
...
async function readOptionalThreadCheckpoint(archiveDirectory) {
  try {
...
function validateThreadCheckpoint(checkpoint, submission, prepared) {
  if (checkpoint === void 0) return;
  if (checkpoint.promptSha256 !== sha256Text2(prepared.prompt)) {
...
async function readArtifactDownloadCheckpoint(archiveDirectory) {
  ...
  const checkpointPath = join8(artifactsDirectory, "download-checkpoint.json");
  const manifestPath = join8(artifactsDirectory, "manifest.json");
  let value;
  try {
...
async function verifyArchivedArtifacts(artifactsDirectory, entries) {
  ...
  for (const entry of entries) {
    ...
    const expectedPath = resolve6(artifactsDirectory, sanitizeArtifactFilename(entry.name));
...
function sameResolvedPath(left, right) {
  const resolvedLeft = resolve6(left);
  const resolvedRight = resolve6(right);
...
function validateRequestedThread(args) {
  ...
  if (args.thread.id !== void 0 && urlId !== void 0 && args.thread.id !== urlId) {
  ...
  if (requestedId === void 0 || isProvisionalConversationId(requestedId)) {
...
async function writeJsonReplacing(path3, value) {
  ...
  await writeFile6(temporary, `${JSON.stringify(value, null, 2)}
`, { flag: "wx", mode: 384 });
  try {
...
var requiredScenarios = [
  ...
  scenario("redacted-run-report", true, () => true, async (context, meta) => {
    ...
    const body = path3 === void 0 ? "" : await readFile7(path3, "utf8").catch(() => "");
...
var optionalScenarios = [
  ...
  scenario("download-generated-file", false, (context) => contextEnvFlag(context, "CHATGPT_E2E_DOWNLOAD"), async (context, meta) => {
    ...
    const content = path3 === void 0 ? "" : await readFile7(path3, "utf8").catch(() => "");
...
function hashPreview(text) {
  ...
  for (const char of text) {
    hash2 = (hash2 << 5) - hash2 + char.charCodeAt(0) | 0;
  }
  return Math.abs(hash2).toString(16);
}
async function tempFile(name, body) {
  const dir = await mkdtemp2(join9(tmpdir2(), "chatgpt-live-smoke-"));
...
async function runReleaseCanary(runtime, options) {
  ...
  const reportDir = resolve7(options.reportDir ?? join10(process.cwd(), "reports", "release-canary"));
  const profileDir = join10(reportDir, "surface-profiles");
  await mkdir8(profileDir, { recursive: true });
  const stamp = (/* @__PURE__ */ new Date()).toISOString().replaceAll(":", "-").replaceAll(".", "-");
  const profilePaths = [
    join10(profileDir, `${stamp}-chat.json`),
    join10(profileDir, `${stamp}-work.json`)
  ];
  try {
  ...
  const context = {
    ...
    reportDir: join10(reportDir, "live-smoke"),
    env: {
