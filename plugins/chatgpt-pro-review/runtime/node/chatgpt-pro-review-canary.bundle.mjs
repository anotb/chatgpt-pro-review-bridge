...
function waitDataFromText(args, complete, responseText, assistantTurnCount, elapsedMs) {
  ...
  if (args.responseContent === "metadata") {
    ...
    data.responseSha256 = createHash3("sha256").update(responseText).digest("hex");
    return data;
  }
...
import { chmod, copyFile as copyFile3, link as link2, mkdir as mkdir4, readFile as readFile3, rm, stat as stat6, writeFile as writeFile3 } from "node:fs/promises";
import { basename as basename3, dirname as dirname2, isAbsolute, join as join4, relative, resolve as resolve3, sep } from "node:path";
...
import { basename as basename4, dirname as dirname3, isAbsolute as isAbsolute2, join as join5, relative as relative2, resolve as resolve4, sep as sep2 } from "node:path";
...
async function validateArchivedTerminalBinding(archiveDirectory, outcome, submission) {
  ...
  if (outcome.response !== void 0) {
    const response = await readFile6(join7(archiveDirectory, "response.md"), "utf8").catch(() => void 0);
...
async function runProReviewCanaryStep(runtime, options = {}) {
  ...
  const promptCount = review.status === "in_progress" || review.thread?.url === void 0 ? 0 : await countVisibleUserPrompts(runtime.browser, review.thread.url);
...
async function countVisibleUserPrompts(browser, threadUrl) {
  ...
  for (const tab of Array.isArray(controlledTabs) ? controlledTabs : []) {
    if (await Promise.resolve(tab.url?.()).catch(() => void 0) === threadUrl) {
...
async function countUserPrompts(page) {
...
async function fullResponseMatchesArchive(review) {
  ...
  const archivedHash = createHash7("sha256").update(archived).digest("hex");
...
function reviewReceipt(review) {
  return {
    ...review,
    responseMarkdown: review.responseMarkdown === void 0 ? void 0 : { bytes: Buffer.byteLength(review.responseMarkdown), sha256: review.provenance.responseSha256 }
  };
}
