...
import { mkdtemp, readFile as readFile6, rename as rename2, rm, writeFile as writeFile4 } from "node:fs/promises";
import { tmpdir } from "node:os";
import { join as join6 } from "node:path";

// src/reviews/archive.ts
import { createHash as createHash5, randomBytes } from "node:crypto";
import { copyFile as copyFile3, mkdir as mkdir4, readFile as readFile4, rename, stat as stat6, writeFile as writeFile3 } from "node:fs/promises";
...
async function writeImmutableFile(path3, data) {
  ...
  await writeFile3(temporary, data, { flag: "wx" });
...
var StdioBackendTransport = class {
  ...
  async request(request) {
    ...
    return new Promise((resolve6, reject) => {
      ...
      this.pendingResponses.set(requestId, { resolve: resolve6, reject, timeout });
...
function streamFromBackendEvents(events) {
  ...
  let rejectCompleted;
  const completed = new Promise((resolve6, reject) => {
    resolveCompleted = resolve6;
