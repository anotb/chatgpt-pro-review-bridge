...
After any plugin upgrade, start a new Codex task. A marketplace installed with
`--ref` does not change refs through `marketplace upgrade`; remove the plugin
and marketplace, add the newer immutable tag, and reinstall it. Existing
in-progress reviews must resume from their original archive directory and
thread URL; do not reconstruct and resend their prompts.
